...
export class OrderService {
    ...
    try {
      ...
      if (!isSeller) {
        throw new ForbiddenException(
          'Only the seller can update order details',
        );
      }
      ...
      const updatableFields = {
        ...
        ...(updateOrderDto.meetupTime && {
          meetupTime: new Date(updateOrderDto.meetupTime),
        }),
      };
  ...
  async updateStatus(orderId: number, status: string, userId: number) {
    try {
      ...
      const updatedOrder = await this.prisma.exchanges.update({
        where: { exchangeId: orderId },
        data: { status: status as exchange_status },
        include: {
          product: {
            include: { users: true },
          },
    ...
    try {
      ...
      if (order.status !== 'pending') {
        throw new BadRequestException(
          'Order can only be accepted when in pending state',
        );
      }
