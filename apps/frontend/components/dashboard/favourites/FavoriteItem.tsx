    ...
    >
      ...
      <div className="relative">
        ...
        <div className="absolute top-3 left-3 flex flex-col space-y-2">
          <span
            className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium border ${getStatusColor(favorite.item.status)}`}
          >
            {favorite.item.status.charAt(0).toUpperCase() +
              favorite.item.status.slice(1)}
          </span>
          <span
            className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium border ${getExchangeTypeColor(favorite.item.exchangeType)}`}
          >
            <Tag className="w-3 h-3" />
            <span>
              {favorite.item.exchangeType === "sale"
                ? "Sale"
                : favorite.item.exchangeType === "exchange"
                  ? "Exchange"
                  : "Free"}
            </span>
          </span>
        ...
        <div className="absolute bottom-3 left-3">
          <span
            className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium border ${getConditionColor(favorite.item.condition)}`}
          >
            {favorite.item.condition.charAt(0).toUpperCase() +
              favorite.item.condition.slice(1)}
          </span>
        </div>
      </div>
      ...
      <div className="p-4">
        {/* Title and Price */}
        <div className="flex items-start justify-between mb-2 h-10">
          <Link href={`http://localhost:3000/shop/product/${favorite.item.id}?category${favorite.item.category}`} className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-100">
          <div className="flex items-center space-x-2">
            {favorite.item.seller.avatar ? (
              ...
              <p className="w-6 h-6 text-center text-gray-500 rounded-full font-bold bg-gradient-to-r from-green-100 to-blue-100">
                {favorite.item.seller.name[0]}
              </p>
            )}
