import { useEffect, useState, useRef, useMemo } from "react";
import { Search, X, Clock, Trash2 } from "lucide-react";
import { authFetch } from "@/lib/authFetch";
import { BACKEND_URL } from "@/lib/constants";
import { useSession } from "@/context/useSession";

interface SearchHistoryDropdownProps {
  searchQuery: string;
  isOpen: boolean;
  setIsOpen: (isOpen: boolean) => void;
  onSelectSearch: (query: string) => void;
  inputRef: React.RefObject<HTMLInputElement>;
}

export default function SearchHistoryDropdown({
  searchQuery,
  isOpen,
  setIsOpen,
  onSelectSearch,
  inputRef,
}: SearchHistoryDropdownProps) {
  const [baseHistory, setBaseHistory] = useState<{ searchid: number; query: string }[]>([]);
  const [apiSuggestions, setApiSuggestions] = useState<{ searchid: number; query: string }[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [isTyping, setIsTyping] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);
  const { session } = useSession();

  // Fetch initial history when dropdown opens
  useEffect(() => {
    if (isOpen && session && baseHistory.length === 0) {
      const fetchHistory = async () => {
        setIsLoading(true);
        try {
          const res = await authFetch(`${BACKEND_URL}/search-history`);
          if (res.ok) {
            const data = await res.json();
            setBaseHistory(data);
          }
        } catch (error) {
          console.error("Failed to fetch search history", error);
        } finally {
          setIsLoading(false);
        }
      };
      fetchHistory();
    }
  }, [isOpen, session, baseHistory.length]);

  // Debounce API suggestions when user types
  useEffect(() => {
    if (!isOpen || !session) return;

    if (searchQuery.trim().length < 2) {
      setApiSuggestions([]);
      setIsTyping(false);
      return;
    }

    setIsTyping(true);
    const controller = new AbortController();

    const fetchSuggestions = async () => {
      try {
        const res = await authFetch(
          `${BACKEND_URL}/search-history/search?q=${encodeURIComponent(searchQuery.trim())}`,
          { signal: controller.signal }
        );
        if (res.ok) {
          const data = await res.json();
          setApiSuggestions(data);
        }
      } catch (error: any) {
        if (error.name !== 'AbortError') {
          console.error("Failed to fetch suggestions", error);
        }
      } finally {
        setIsTyping(false);
      }
    };

    const debounceTimer = setTimeout(() => {
      fetchSuggestions();
    }, 300);

    return () => {
      clearTimeout(debounceTimer);
      controller.abort();
    };
  }, [searchQuery, isOpen, session]);

  // Merge client-side filtered history and API suggestions
  const displayResults = useMemo(() => {
    if (!searchQuery.trim()) {
      return baseHistory;
    }

    const queryLower = searchQuery.trim().toLowerCase();
    
    // 1. Client-side filtering of already fetched history
    const clientFiltered = baseHistory.filter(item => 
      item.query?.toLowerCase().includes(queryLower)
    );

    // 2. Merge with API suggestions, avoiding duplicates
    const combinedMap = new Map();
    
    clientFiltered.forEach(item => combinedMap.set(item.query.toLowerCase(), item));
    apiSuggestions.forEach(item => combinedMap.set(item.query.toLowerCase(), item));

    // Convert map values to array
    const combinedArray = Array.from(combinedMap.values());

    // 3. Sort logic: Exact match -> Starts with -> Contains
    combinedArray.sort((a, b) => {
      const aLower = a.query.toLowerCase();
      const bLower = b.query.toLowerCase();

      const aExact = aLower === queryLower ? 1 : 0;
      const bExact = bLower === queryLower ? 1 : 0;
      if (aExact !== bExact) return bExact - aExact;

      const aStarts = aLower.startsWith(queryLower) ? 1 : 0;
      const bStarts = bLower.startsWith(queryLower) ? 1 : 0;
      if (aStarts !== bStarts) return bStarts - aStarts;

      return 0; // maintain original relative order otherwise
    });

    return combinedArray.slice(0, 10);
  }, [baseHistory, apiSuggestions, searchQuery]);

  // Handle clicking outside to close
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (
        dropdownRef.current &&
        !dropdownRef.current.contains(e.target as Node) &&
        inputRef.current &&
        !inputRef.current.contains(e.target as Node)
      ) {
        setIsOpen(false);
      }
    };
    document.addEventListener("mousedown", handleClickOutside);
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, [setIsOpen, inputRef]);

  const handleDelete = async (e: React.MouseEvent, id: number) => {
    e.stopPropagation();
    try {
      const res = await authFetch(`${BACKEND_URL}/search-history/${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
        setBaseHistory((prev) => prev.filter((item) => item.searchid !== id));
        setApiSuggestions((prev) => prev.filter((item) => item.searchid !== id));
      }
    } catch (error) {
      console.error("Failed to delete search history item", error);
    }
  };

  const handleClearAll = async (e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      const res = await authFetch(`${BACKEND_URL}/search-history`, {
        method: "DELETE",
      });
      if (res.ok) {
        setBaseHistory([]);
        setApiSuggestions([]);
      }
    } catch (error) {
      console.error("Failed to clear search history", error);
    }
  };

  // Helper to highlight matching text
  const HighlightMatch = ({ text, highlight }: { text: string; highlight: string }) => {
    if (!highlight.trim()) return <span>{text}</span>;
    
    const regex = new RegExp(`(${highlight})`, 'gi');
    const parts = text.split(regex);
    
    return (
      <span>
        {parts.map((part, i) => 
          regex.test(part) ? (
            <span key={i} className="font-bold text-green-600 bg-green-50">{part}</span>
          ) : (
            <span key={i}>{part}</span>
          )
        )}
      </span>
    );
  };

  if (!isOpen) return null;
  if (!session) return null;

  return (
    <div
      ref={dropdownRef}
      className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-xl shadow-lg z-50 overflow-hidden"
    >
      <div className="p-3">
        <div className="flex justify-between items-center mb-2 px-2">
          <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
            {searchQuery.trim() ? "Search Suggestions" : "Recent Searches"}
          </span>
          {!searchQuery.trim() && baseHistory.length > 0 && (
            <button
              onClick={handleClearAll}
              className="text-xs text-red-500 hover:text-red-700 font-medium flex items-center transition-colors"
            >
              Clear All
            </button>
          )}
        </div>

        {isLoading && !searchQuery.trim() ? (
          <div className="p-4 text-center text-gray-500 text-sm">Loading...</div>
        ) : displayResults.length === 0 ? (
          <div className="p-4 text-center text-gray-500 text-sm">
            {searchQuery.trim() ? "No matching searches" : "No recent searches"}
          </div>
        ) : (
          <ul className="max-h-60 overflow-y-auto">
            {displayResults.map((item) => (
              <li
                key={item.searchid}
                className="flex items-center justify-between p-2 hover:bg-gray-50 rounded-lg cursor-pointer transition-colors group"
                onClick={() => {
                  onSelectSearch(item.query);
                  setIsOpen(false);
                }}
              >
                <div className="flex items-center space-x-3 text-gray-700">
                  <Search className="w-4 h-4 text-gray-400" />
                  <span className="text-sm font-medium">
                    <HighlightMatch text={item.query} highlight={searchQuery.trim()} />
                  </span>
                </div>
                {!searchQuery.trim() && (
                  <button
                    onClick={(e) => handleDelete(e, item.searchid)}
                    className="p-1 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all rounded-full hover:bg-red-50"
                    title="Remove from history"
                  >
                    <X className="w-4 h-4" />
                  </button>
                )}
              </li>
            ))}
          </ul>
        )}
      </div>
    </div>
  );
}
